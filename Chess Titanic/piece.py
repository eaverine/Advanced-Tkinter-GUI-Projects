...
def create_piece(piece, color = 'white'):
    if isinstance(piece, str):
        ...
        if piece in SHORT_NAME.values():
            return eval(f'{piece} (color)')

    raise ChessError(f'Invalid piece name: {piece}')
...
class Piece():
    ...
    def moves_available(self, current_position, directions, distance):
        ...
        allowed_moves = []
        piece = self
        start_row, start_col = get_numeric_notation(current_position)
        ...
        for x, y in directions:
            ...
            for step in range(1, distance + 1):
                if collision:
                    break
                destination = start_row + step * x, start_col + step * y
...
class Pawn(Piece):
    def moves_available(self, current_position):
        ...
        piece = self
        if self.color == 'black':
            inital_position, direction, enemy = 1, 1, 'white'
        else:
            inital_position, direction, enemy = 6, -1, 'black'
        ...
        if model.get_alphanumeric_position(forward) not in prohibited:
            ...
            if start_position[0] == inital_position:
